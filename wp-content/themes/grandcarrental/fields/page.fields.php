...
function grandcarrental_page_create_meta_box() {
	...
	if ( function_exists('add_meta_box') && isset($grandcarrental_page_postmetas) && count($grandcarrental_page_postmetas) > 0 ) {  
		add_meta_box( 'page_metabox', 'Page Options', 'grandcarrental_page_new_meta_box', 'page', 'normal', 'default' );  
	}

}  

function grandcarrental_page_new_meta_box() {
	$post = grandcarrental_get_wp_post();
	$grandcarrental_page_postmetas = grandcarrental_get_page_postmetas();
	
	//Get page template sample URL
	$page_template_url = array (
	  'Default Template' 						=> grandcarrental_get_demo_url('/pages/page-fullwidth/'),
	  'Blog Fullwidth' 							=> grandcarrental_get_demo_url('/blog/blog-fullwidth/'),
	  'Blog Full + Grid Fullwidth' 				=> grandcarrental_get_demo_url('/blog/blog-full-post-grid-fullwidth/'),
	  'Blog Full + Grid Left Sidebar' 			=> grandcarrental_get_demo_url('/blog/blog-full-post-grid-left-sidebar/'),
	  'Blog Full + Grid Right Sidebar' 			=> grandcarrental_get_demo_url('/blog/blog-full-post-grid-right-sidebar/'),
	  'Blog Grid Fullwidth' 					=> grandcarrental_get_demo_url('/blog/blog-grid-fullwidth/'),
	  'Blog Grid Left Sidebar' 					=> grandcarrental_get_demo_url('/blog/blog-grid-left-sidebar/'),
	  'Blog Grid Right Sidebar' 				=> grandcarrental_get_demo_url('/blog/blog-grid-right-sidebar/'),
	  'Blog Left Sidebar' 						=> grandcarrental_get_demo_url('/blog/blog-left-sidebar/'),
	  'Blog Right Sidebar' 						=> grandcarrental_get_demo_url('/blog/blog-right-sidebar/'),
	  'Car 2 Columns Classic' 					=> grandcarrental_get_demo_url('/car-2-columns-classic/'),
	  'Car 3 Columns Classic' 					=> grandcarrental_get_demo_url('/car-3-columns-classic/'),
	  'Car 4 Columns Classic' 					=> grandcarrental_get_demo_url('/car-4-columns-classic/'),
	  'Car Classic Right Sidebar' 				=> grandcarrental_get_demo_url('/car-classic-right-sidebar/'),
	  'Car Classic Left Sidebar' 				=> grandcarrental_get_demo_url('/car-classic-left-sidebar/'),
	  'Car 2 Columns Grid' 						=> grandcarrental_get_demo_url('/car-2-columns-grid/'),
	  'Car 3 Columns Grid' 						=> grandcarrental_get_demo_url('/car-3-columns-grid/'),
	  'Car 4 Columns Grid' 						=> grandcarrental_get_demo_url('/car-4-columns-grid/'),
	  'Car Grid Right Sidebar' 					=> grandcarrental_get_demo_url('/car-grid-right-sidebar/'),
	  'Car Grid Left Sidebar' 					=> grandcarrental_get_demo_url('/car-grid-left-sidebar/'),
	  'Car List Thumbnail Right Sidebar' 		=> grandcarrental_get_demo_url('/car-list-thumbnail-right-sidebar/'),
	  'Car List Thumbnail Left Sidebar' 		=> grandcarrental_get_demo_url('/car-list-thumbnail-left-sidebar/'),
	  'Car List Right Sidebar' 					=> grandcarrental_get_demo_url('/car-list-right-sidebar/'),
	  'Car List Left Sidebar' 					=> grandcarrental_get_demo_url('/car-list-left-sidebar/'),
	  'Car By Brand & Type Fullwidth'			=> grandcarrental_get_demo_url('/car-by-brand-type-fullwidth/'),

	  'Page Left Sidebar' 						=> grandcarrental_get_demo_url('/pages/page-left-sidebar/'),
	  'Page Right Sidebar' 						=> grandcarrental_get_demo_url('/pages/page-right-sidebar/'),
	);

	echo '<input type="hidden" name="pp_meta_form" id="pp_meta_form" value="' . wp_create_nonce('grandcarrental_once') . '" />';
	
	$meta_section = '';
	$key = 0;
	foreach ( $grandcarrental_page_postmetas as $key => $postmeta ) {
...
function grandcarrental_page_save_postdata( $post_id ) {
	...
	if ( isset($_POST['pp_meta_form']) && !wp_verify_nonce( $_POST['pp_meta_form'], 'grandcarrental_once' )) {
		return $post_id;
	}
	...
	if (isset($_POST['pp_meta_form'])) 
	{
		...
		if(is_admin() && isset($_POST['ppb_import_current']) && !empty($_POST['ppb_import_current']))
		{
			...
			else
			{
				...
				{
					$import_options_json = get_option( GRANDCARRENTAL_SHORTNAME."_template_".$_POST['ppb_import_template_key']);
				}
				//If upload import builder file
				else
				{
		...
		foreach ( $grandcarrental_page_postmetas as $postmeta ) 
		{
		
			if (isset($_POST[$postmeta['id']]) && $_POST[$postmeta['id']]) {
				grandcarrental_page_update_custom_meta($post_id, $_POST[$postmeta['id']], $postmeta['id']);
			}
		...
		{
			grandcarrental_page_update_custom_meta($post_id, $_POST['ppb_enable'], 'ppb_enable');
		}
		else
		{
		...
		{
			grandcarrental_page_update_custom_meta($post_id, $_POST['ppb_form_data_order'], 'ppb_form_data_order');
			...
			if(is_array($ppb_item_arr) && !empty($ppb_item_arr))
			{
				foreach($ppb_item_arr as $key => $ppb_item_arr)
				{
					...
					{
						grandcarrental_page_update_custom_meta($post_id, $_POST[$ppb_item_arr.'_data'], $ppb_item_arr.'_data');
					}
					
					if(isset($_POST[$ppb_item_arr.'_size']) && !empty($_POST[$ppb_item_arr.'_size']))
					{
						grandcarrental_page_update_custom_meta($post_id, $_POST[$ppb_item_arr.'_size'], $ppb_item_arr.'_size');
					}
				}
			}
		}
		...
		{
			grandcarrental_page_update_custom_meta($post_id, '', 'ppb_form_data_order');
		}
	}
	...
	if (isset($_POST['ppb_enable']) && !empty($_POST['ppb_enable']) && ! wp_is_post_revision( $post_id ) )
	{
		...
		remove_action('save_post', 'grandcarrental_page_save_postdata');
	
		//update the post, which calls save_post again
		$ppb_page_content = grandcarrental_apply_builder($post_id, 'page', FALSE);
		...
		add_action('save_post', 'grandcarrental_page_save_postdata');
	}

}

function grandcarrental_page_update_custom_meta($postID, $newvalue, $field_name) {
...
add_action('admin_menu', 'grandcarrental_page_create_meta_box'); 
add_action('save_post', 'grandcarrental_page_save_postdata');  
