...
function grandcarrental_add_admin() 
{
	...
	if ( isset($_GET['page']) && $_GET['page'] == 'functions.php' ) {
		...
		if ( isset($_REQUEST['action']) && 'save' == $_REQUEST['action'] ) {
				...
				if(!empty($_REQUEST['pp_envato_personal_token']) && strlen($_REQUEST['pp_envato_personal_token']) == 36) {
					...
					if(is_bool($response_obj->response_code)) {
						if($response_obj->response_code) {
							...
							grandcarrental_register_theme($_REQUEST['pp_envato_personal_token']);
				...
				if(!empty($_REQUEST['pp_envato_personal_token']) && strlen($_REQUEST['pp_envato_personal_token']) == 36) {
					...
					if(is_bool($response_obj->response_code)) {
						if($response_obj->response_code) {
							...
							grandcarrental_unregister_theme();
			...
			{
				$css_cache_path = GRANDCARRENTAL_THEMEUPLOAD.'/custom-css.css';
				...
				if(!file_exists($css_cache_path))
				{
					...
					$grandcarrental_custom_css = ob_get_contents(); 
					ob_end_clean();
					
					//Writing cache file
					file_put_contents($css_cache_path, $grandcarrental_custom_css);
				}
			}
...
function grandcarrental_enqueue_front_page_scripts() 
{
	...
	{
		$cache_url = GRANDCARRENTAL_THEMEUPLOADURL;
	}
	...
	else
	{
		...
		if($advance_cache_custom_css)
		{
			...
			wp_enqueue_style("grandcarrental-custom-css", $cache_url."/custom-css.css", false, "", "all");
		}
		else
		{
		...
		<?php 
			foreach ($grandcarrental_options as $value) {
				...
				if($value['type'] == 'section')
				{
					...
					echo '<a id="pp_panel_'.esc_attr(strtolower($value['name'])).'_a" href="#pp_panel_'.esc_attr(strtolower($value['name'])).'" class="nav-tab '.esc_attr($active).'">'.str_replace('-', ' ', esc_html($value['name'])).'</a>';
				}
			}
		?>
		</h2>
...
function grandcarrental_add_to_cart() {
	if(isset($_GET['product_id']) && !empty($_GET['product_id']) && class_exists('Woocommerce'))
	{
		...
		$book_date_range = $book_start_date . " to " . $book_end_date;  
		...
		if ($item_exists) {
			$woocommerce->cart->remove_cart_item($current_item_key);
			$woocommerce->cart->add_to_cart($current_product_id, $days_count, 0, array(), array('product_custom_data' => array('Date' => $book_date_range)));
		} else {
			$woocommerce->cart->add_to_cart($current_product_id, $days_count, 0, array(), array('product_custom_data' => array('Date' => $book_date_range)));
		}
	}
...
function display_cart_item_custom_meta_data( $item_data, $cart_item ) {
    $meta_key = 'Date';
    if ( isset($cart_item['product_custom_data']) && isset($cart_item['product_custom_data'][$meta_key]) ) {
        $item_data[] = array(
            'key'       => $meta_key,
            'value'     => $cart_item['product_custom_data'][$meta_key],
        );
    }
    return $item_data;
}
...
function save_cart_item_custom_meta_as_order_item_meta( $item, $cart_item_key, $values, $order ) {
    $meta_key = 'Date';
    if ( isset($values['product_custom_data']) && isset($values['product_custom_data'][$meta_key]) ) {
        $item->update_meta_data( $meta_key, $values['product_custom_data'][$meta_key] );
    }
}
...
// Add custom checkbox after terms and conditions
add_action('woocommerce_checkout_after_terms_and_conditions', 'add_custom_checkbox_text');
function add_custom_checkbox_text() {
    woocommerce_form_field('custom_note_agreement', array(
        'type'          => 'checkbox',
        'class'         => array('form-row custom-note-agreement'),
        'label_class'   => array('woocommerce-form__label-for-checkbox checkbox'),
        'input_class'   => array('woocommerce-form__input woocommerce-form__input-checkbox input-checkbox'),
        'required'      => true,
        'label'         => 'I understand that if the car is unavailable, I will receive a full refund unless I agree to a car of the same model or brand.',
    ));
}
...
function validate_custom_checkbox() {
    if (empty($_POST['custom_note_agreement'])) {
        wc_add_notice(__('Please acknowledge our availability policy by checking the box.'), 'error');
    }
}

// Save the custom checkbox value as order meta
add_action('woocommerce_checkout_create_order', 'save_custom_checkbox_value');
function save_custom_checkbox_value($order) {
    if (!empty($_POST['custom_note_agreement'])) {
        $order->update_meta_data('_custom_note_agreement', '1');
        $order->add_order_note('Customer acknowledged the availability policy.');
    }
}
