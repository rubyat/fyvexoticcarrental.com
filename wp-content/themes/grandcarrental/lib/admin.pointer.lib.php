...
add_action(
	...
	function() {
		...
		if (
			! get_user_meta(
				get_current_user_id(),
				'grandcarrental-pointer-registration-dismissed',
				true
			)
		):
		...
		?>
			...
			jQuery(
				function() {
					jQuery('#setting-panel-registration-a').first().pointer( 
						{
							content:
							...
							close: function() {
								jQuery.post(
									ajaxurl,
									{
										pointer: 'grandcarrental-pointer-registration',
...
add_action(
	...
	function() {
		...
		if (
			! get_user_meta(
				get_current_user_id(),
				'grandcarrental-pointer-required-plugins-dismissed',
				true
			)
		):
					...
					jQuery('#menu-appearance').first().pointer( 
						{
							content:
								...
								"<p>Please see detailed instruction <a href='https://docs.themegoods.com/docs/grandcarrental/getting-started/register-your-purchase-using-token-key/' target='_blank'>here</a>.</p>",
							...
							close: function() {
								jQuery.post(
									ajaxurl,
									{
										pointer: 'grandcarrental-pointer-required-plugins',
	...
	add_action(
		...
		function() {
			...
			if (
				! get_user_meta(
					get_current_user_id(),
					'grandcarrental-pointer-loftloader-dismissed',
					true
				)
			):
						...
						jQuery('#menu-settings').first().pointer( 
							{
								...
								close: function() {
									jQuery.post(
										ajaxurl,
										{
											pointer: 'grandcarrental-pointer-loftloader',
