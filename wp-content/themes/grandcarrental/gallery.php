...
$grandcarrental_page_gallery_id = grandcarrental_get_page_gallery_id();
if(!empty($grandcarrental_page_gallery_id))
{
	$current_page_id = $grandcarrental_page_gallery_id;
}
...
//Sort gallery images
$all_photo_arr = grandcarrental_resort_gallery_img($all_photo_arr);
	...
	<?php
	    ...
	    foreach($all_photo_arr as $key => $photo_id)
	    {
	        ...
	        if(!empty($photo_id))
	        {
	        	...
	        	$small_image_url = wp_get_attachment_image_src($photo_id, 'grandcarrental-gallery-list', true);
	        }
			...
			?>		
			    <a <?php if(!empty($tg_lightbox_enable_caption)) { ?>data-caption="<?php if(!empty($image_caption)) { ?><?php echo esc_attr($image_caption); ?><?php } ?>"<?php } ?> class="fancy-gallery" href="<?php echo esc_url($image_url[0]); ?>">
			        ...
			        <?php
					     ...
					     {
					 ?>
					 <a href="<?php echo esc_url($grandcarrental_purchase_url); ?>" title="<?php echo esc_html__('Purchase', 'grandcarrental' ); ?>" class="button tooltip"><i class="fa fa-shopping-cart"></i></a>
					 <?php
					     }
					 ?>
