jQuery(document).ready(function(){ 
	...
	jQuery('.single_car_add_to_cart').on( 'click', function(e) {
		e.preventDefault();
		// Validate Booking Start and End Date fields
		jQuery('.book-start-date-err-elt').html('');
		jQuery('.book-end-date-err-elt').html('');
		var book_start_date = jQuery("input[name='book-start-date']").val();
		var book_end_date = jQuery("input[name='book-end-date']").val();
		
		if (!book_start_date) {
			jQuery('.book-start-date-err-elt').html('The field is required.').fadeIn();
			return false;
		} else if (!book_end_date) {
			jQuery('.book-end-date-err-elt').html('The field is required.').fadeIn();
			return false;
		} else if(book_start_date >= book_end_date) {
			jQuery('.book-end-date-err-elt').html('The end date must be a date after the start date.').fadeIn();
			return false;
		}
		var formData = {book_start_date: book_start_date, book_end_date: book_end_date}; //Array 
	...
	// Make quantity fields as readonly
	jQuery('.product-quantity .qty').prop('readonly', true);
